...
function AddProduct() {
    ...
    const handleApi = () => {
        if (!isFormValid) {
            alert('All fields are required except price and one image.');
            return;
        }
            ...
            axios.post(url, fromData)
                .then((res) => {
                    ...
                    if (res.data.message) {
                        ...
                        // 📌 Add a notification when a product is added
                        axios.post(API_URL + '/add-notification', {
                            userId: localStorage.getItem('userId'),
                            message: `You added a new product: ${pname}`,
                            productId: res.data.productId // Assuming API returns productId
                        });
                        navigate('/home');
                    }
                })
                    ...
                    <div className="col-md-6">
                        <div className="card shadow-lg p-4">
                            ...
                            <div className="mb-3">
                                ...
                                <input className="form-control" type="text" value={pname} onChange={(e) => setpname(e.target.value)} required />
                            </div>
                            <div className="mb-3">
                                <label className="form-label">Product Description</label>
                                <input className="form-control" type="text" value={pdesc} onChange={(e) => setpdesc(e.target.value)} required />
                            </div>
                            <div className="mb-3">
                                <label className="form-label">Product Price (Optional)</label>
